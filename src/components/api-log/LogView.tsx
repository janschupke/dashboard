    ...
    if (isOpen) {
      ...
      if (mainElement) {
        ...
        const preventWheel = (e: WheelEvent) => {
          ...
          if (logViewElement && logViewElement.contains(e.target as Node)) {
        ...
        mainElement.addEventListener('touchmove', preventWheel, { passive: false, capture: true });

        return () => {
          mainElement.style.overflow = '';
          mainElement.removeEventListener('wheel', preventWheel, { capture: true });
          mainElement.removeEventListener('touchmove', preventWheel, { capture: true });
        };
      }
    } else {
      const mainElement = document.querySelector('main');
  ...
  const columns = useMemo<ColumnDef<APILogEntry>[]>(
    () => [
      ...
      },
      {
        ...
        header: 'Time',
        cell: ({ row }) => {
          return <span className="text-sm text-primary">{formatTimestamp(row.original.timestamp)}</span>;
        },
      },
      {
        accessorKey: 'apiCall',
        ...
        cell: ({ row }) => {
          return (
            <span className="text-sm text-primary font-mono" data-testid={`log-row-${row.original.id}`}>
  ...
  const table = useReactTable({
    ...
    onExpandedChange: (updater) => {
      if (typeof updater === 'function') {
        ...
        setExpandedIds(new Set(Object.keys(newExpanded).filter((key) => newExpanded[key])));
      } else {
        setExpandedIds(new Set(Object.keys(updater).filter((key) => updater[key])));
      }
    },
  });
                    ...
                    <React.Fragment key={row.id}>
                      ...
                      {row.original.details && expandedIds.has(row.original.id) && (
                        <tr id={`log-details-${row.original.id}`}>
                          <td colSpan={table.getAllColumns().length} className="px-4 pb-4 pt-0 text-xs text-secondary bg-surface-secondary">
