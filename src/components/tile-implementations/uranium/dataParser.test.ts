...
describe('UraniumHtmlDataParser', () => {
  ...
  describe('validate', () => {
    it('should validate HTML with uranium price content', () => {
      ...
      const validHtml = '<html><head><meta name="description" content="Uranium rose to 85.5 USD/Lbs" /></head><body><span id="spot-price">85.5</span><td id="p">85.5</td><div>Uranium price information and market data for trading economics and financial analysis purposes.</div></body></html>';
    ...
    it('should accept HTML with uranium and price content', () => {
      const htmlWithUranium = '<html><head><meta name="description" content="Uranium rose to 85.5 USD/Lbs" /></head><body>Uranium price is 85.5</body></html>';
  ...
  describe('parse', () => {
    ...
    it('should parse HTML with #p selector (Trading Economics format)', () => {
      ...
      const html = '<html><head><meta name="description" content="Uranium price 81.95 USD/Lbs" /></head><body><table><tr><td id="p">81.95</td></tr></table><div>Uranium market data and trading information for financial analysis purposes with additional content to meet length requirements.</div></body></html>';
      const result = parser.parse(html);
    ...
    it('should extract price from meta description', () => {
      const html = '<html><head><meta name="description" content="Uranium rose to 85.5 USD/Lbs" /></head><body></body></html>';
      const result = parser.parse(html);
    ...
    it('should extract change percentage from meta description', () => {
      const html = '<html><head><meta name="description" content="Uranium rose to 85.5 USD/Lbs, up 0.68% from the previous day" /></head><body></body></html>';
      const result = parser.parse(html);
    ...
    it('should throw error when price cannot be extracted', () => {
      ...
      expect(() => parser.parse(htmlWithoutPrice)).toThrow('Could not extract uranium spot price from HTML');
    });
  });
  ...
  describe('safeParse', () => {
    it('should successfully parse valid HTML', () => {
      ...
      const html = '<html><head><meta name="description" content="Uranium rose to 85.5 USD/Lbs" /></head><body><span id="spot-price">85.5</span><div>Uranium market information</div></body></html>';
    ...
    it('should throw error when parsing fails', () => {
      ...
      const htmlWithoutPrice = '<html><head><meta name="description" content="Uranium information 50000" /></head><body><div>Uranium market data with large numbers. Some additional content here to ensure the HTML is long enough to pass validation checks and contains uranium keyword as required for proper validation.</div></body></html>';
