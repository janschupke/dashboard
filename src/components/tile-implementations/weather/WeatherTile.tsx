...
const WeatherMetrics = memo(({ data }: { data: WeatherTileData }): React.ReactNode => {
...
const WeatherForecast = memo(
  ({ daily, showForecast }: { daily: WeatherTileData['daily']; showForecast: boolean }) => {
          ...
          {daily.slice(1, 6).map((day, index) => (
            ...
            >
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs text-tertiary">{Math.round(day.temperature.min)}°</span>
              </div>
            </div>
          ))}
        </div>
    ...
    return (
      <div className="flex flex-col h-full p-2">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <div className="text-xs text-secondary capitalize">{data.conditions.description}</div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={onToggleForecast} className="text-xs text-tertiary hover:text-secondary">
...
export const WeatherTile = ({
  ...
  const { data, status, lastUpdated, lastSuccessfulDataUpdate, manualRefresh, isLoading } =
    useTileData(getWeather, tile.id, pathParams, queryParams);
